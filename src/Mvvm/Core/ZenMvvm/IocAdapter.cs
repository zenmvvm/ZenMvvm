...
namespace ZenMvvm
{
    ...
    public class IocAdapter : IIoc
    {
        ...
        public IocAdapter(object container, string resolveMethod, string interfaceName = null)
        {
            this.container = container;
            this.interfaceName = interfaceName;
            this.resolveMethod = container.GetType().GetMethod(resolveMethod, new[] { typeof(Type) }) ?? throw new ArgumentException(nameof(IocAdapter)+" could not attach to provided method '"+resolveMethod+"'");
        }
        ...
        public IocAdapter(object container, Type resolutionExtensionType, string resolveMethod)
        {
            this.isExtensionMethod = true;
            this.container = container;
            this.resolveMethod = resolutionExtensionType.GetMethod(resolveMethod, new[] { container.GetType(), typeof(Type) }) ?? throw new ArgumentException(nameof(IocAdapter) + " could not attach to provided method '" + resolveMethod + "'");
        }
